...
if ('All' %in% arg$names){
	names <- c('expgrow','expdecay','log','logrev','gaussian','invgaussian','poslinear','neglinear','noshape','normalb','zerob','normalblowainc','zeroblowainc','fcipost','fmcethpost')
}else {
	...
	for (nit in nitems){
		for (nst in nstud){
			for (r in nrun){
				...
				ggplot(data=scoredf, mapping=aes(x=Raw.Perc,y=WS.Perc))+geom_point(size=1)+labs(title=paste0('Weighted Score vs Raw Score'))+scale_x_continuous(name='Raw Score', n.breaks=10, limits=c(0,1))+scale_y_continuous(name='Weighted Score', n.breaks=10, limits=c(0,1))+annotate('segment', x=0, y=0, xend=1, yend=1, colour='blue', linetype='dashed')+geom_line(data=meandf, aes(x=Raw.Perc,y=Mean.WS.Perc), color='red')
				if (grepl('fci', name) | grepl('fmce', name)){
					ggsave(file=paste0('WeightedScvRawSc-',paste0(name,r),'.pdf'), path=paste0('comparescoresout/',name,'/'))
				}else {
					ggsave(file=paste0('WeightedScvRawSc-',paste0(name,r),'.pdf'), path=paste0('comparescoresout/',name,'/',nit,'items','/',nst,'students','/'))
				}
				
				ggplot(data=scoredf, mapping=aes(x=Raw.Perc,y=Percent.Difference))+geom_point(size=1)+labs(title=paste0('Percent Difference vs Raw Score'))+scale_x_continuous(name='Raw Score', n.breaks=10, limits=c(0,1))+scale_y_continuous(name='Percent Difference: Weighted Score - Raw Score', n.breaks=10)+annotate('segment', x = 0, xend = 1, y=0, colour='blue', linetype='dashed')+geom_line(data=meandf, aes(x=Raw.Perc,y=Mean.Percent.Difference), color='red')
				if (grepl('fci', name) | grepl('fmce', name)){
					ggsave(file=paste0('PercDiffvRawSc-',paste0(name,r),'.pdf'), path=paste0('comparescoresout/',name,'/'))
				}else {
